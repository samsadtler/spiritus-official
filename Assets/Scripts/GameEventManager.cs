...
// ***************** New Bluetooth Code 2/23/16 *****************
	string fromArduino = "" ;
	string stringToEdit = "baka"; //HC-05
//	string stringToEdit2 = "Adafruit Ez-Link 74ae"; //HC-05
// ***************** New Bluetooth Code 2/23/16 *****************
...
//			// changed RelieverNormal to RelieverRest for open state
...
//			BtConnector.moduleName(stringToEdit2);
			BtConnector.connect();
...
//			jo.Call("stopCommunication");
	...
	// send message to device, This is where I believe new devices should be added
	// I suppose the connection should be confirmed first
	// Not sure why there is a loop in the command
	// Maybe to ensure it sends
	IEnumerator MessageReliever(string command){
		Debug.Log("Sending Commmand: " + command);
		for(int i = 0; i < 4; i++){
			//			jo.Call("writeString", "baka", command);
// ***************** New Bluetooth Code 2/23/16 *****************
			BtConnector.sendString(command);
// ***************** New Bluetooth Code 2/23/16 *****************

			//			jo.Call("writeString", "aho", command);
